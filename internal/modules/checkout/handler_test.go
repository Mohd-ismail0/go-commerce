package checkout

import (
	"bytes"
	"net/http"
	"net/http/httptest"
	"testing"

	"github.com/go-chi/chi/v5"
	"rewrite/internal/shared/events"
	"rewrite/internal/shared/middleware"
)

func TestCheckoutCreateRequiresIdempotencyKey(t *testing.T) {
	h := NewHandler(NewService(&fakeRepo{}, events.NewBus(), nil))
	r := chi.NewRouter()
	r.Use(middleware.TenantRegion("public", "global"))
	h.RegisterRoutes(r)

	req := httptest.NewRequest(http.MethodPost, "/checkouts/sessions", bytes.NewBufferString(`{"customer_id":"c1","currency":"USD"}`))
	req.Header.Set("X-Tenant-ID", "tenant_a")
	req.Header.Set("X-Region-ID", "global")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)

	if rr.Code != http.StatusBadRequest {
		t.Fatalf("expected status 400, got %d", rr.Code)
	}
}

func TestCheckoutUpsertLineRequiresIdempotencyKey(t *testing.T) {
	h := NewHandler(NewService(&fakeRepo{}, events.NewBus(), nil))
	r := chi.NewRouter()
	r.Use(middleware.TenantRegion("public", "global"))
	h.RegisterRoutes(r)

	req := httptest.NewRequest(http.MethodPut, "/checkouts/sessions/chk_1/lines", bytes.NewBufferString(`{"id":"ln_1","quantity":1,"unit_price_cents":100,"currency":"USD","variant_id":"var_ok"}`))
	req.Header.Set("X-Tenant-ID", "tenant_a")
	req.Header.Set("X-Region-ID", "global")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)

	if rr.Code != http.StatusBadRequest {
		t.Fatalf("expected status 400, got %d", rr.Code)
	}
}

func TestCheckoutRecalculateRequiresIdempotencyKey(t *testing.T) {
	h := NewHandler(NewService(&fakeRepo{}, events.NewBus(), nil))
	r := chi.NewRouter()
	r.Use(middleware.TenantRegion("public", "global"))
	h.RegisterRoutes(r)

	req := httptest.NewRequest(http.MethodPost, "/checkouts/sessions/chk_1/recalculate", nil)
	req.Header.Set("X-Tenant-ID", "tenant_a")
	req.Header.Set("X-Region-ID", "global")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)

	if rr.Code != http.StatusBadRequest {
		t.Fatalf("expected status 400, got %d", rr.Code)
	}
}

func TestCheckoutApplyCustomerAddressesRequiresIdempotencyKey(t *testing.T) {
	h := NewHandler(NewService(&fakeRepo{}, events.NewBus(), nil))
	r := chi.NewRouter()
	r.Use(middleware.TenantRegion("public", "global"))
	h.RegisterRoutes(r)

	req := httptest.NewRequest(http.MethodPost, "/checkouts/sessions/chk_1/apply-customer-addresses", bytes.NewBufferString(`{"shipping_address_id":"adr_1"}`))
	req.Header.Set("X-Tenant-ID", "tenant_a")
	req.Header.Set("X-Region-ID", "global")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)

	if rr.Code != http.StatusBadRequest {
		t.Fatalf("expected status 400, got %d", rr.Code)
	}
}

func TestCheckoutCompleteRequiresIdempotencyKey(t *testing.T) {
	h := NewHandler(NewService(&fakeRepo{}, events.NewBus(), nil))
	r := chi.NewRouter()
	r.Use(middleware.TenantRegion("public", "global"))
	h.RegisterRoutes(r)

	req := httptest.NewRequest(http.MethodPost, "/checkouts/sessions/chk_1/complete", nil)
	req.Header.Set("X-Tenant-ID", "tenant_a")
	req.Header.Set("X-Region-ID", "global")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)

	if rr.Code != http.StatusBadRequest {
		t.Fatalf("expected status 400, got %d", rr.Code)
	}
}
